...
void loop() {
    // we gotta alternate between processing messages to and from the Cloud... keeps connectivity with cubetube alive
    Spark.process();
	// do our thingy
  	FFTJoy();
	frameCount++;
 ...
 * *****************************************/
 void FFTJoy(){
    ...
    for(int i=0;i<pow(2,M)/2;i++)
    {
		...
		for(;y<cube.size;y++){
            ...
            if(smoothLEDs){
                ...
                //fade the pixels as the audio level drops; creates a nice and smooth 'trailing' effect
	...
	max= (max>=120) ? max-2 : (max<8) ? 8 : max-0.8;
    //sample++;
    //if(sample>=pow(2,M))
    //    sample-=pow(2,M);
}
