...
{
  float b = a/b;
  if (b >= 0) return float(int(b));
  else return float(int(-b));
}
...
void loop()
{
  ...
  for(int x = 0; x < maxSize; x++)
  {
    ...
    {
        z = sin_32(phase + sqrt(pow(map(toFloat(x),toFloat(0),toFloat(maxSize-1),-PI,PI),2) + pow(map(toFloat(y),toFloat(0), toFloat(maxSize-1),-PI,PI),2)));		  
            int zm = int(map(toFloat(z),toFloat(-1),toFloat(1),toFloat(0),toFloat(maxSize-1)));
      //z = (int(phase)%maxSize + x) /2;
      
        cube.setVoxel(x, zm, y, Color(int(phase)%255,150,100));
    }
}
 cube.setVoxel(iterations%8, iterations%64, iterations%512, red); 
