...
int vizId=140;
Cube cube=Cube();
  //this is a yellow color, but I deliberately keep it at ~20% brightness
...
void loop()
{
  ...
  for(int x=0;x<cube.size;x++)
    	for(int y=0;y<cube.size;y++)
            ...
            {
              	cube.setVoxel(x,y,z,Color(rand()%-55,55,0););
              	cube.show();  //display the cube with this current voxel (and all the voxels before it) colored yellow
              	delay(100);   //wait 0.01 seconds before advancing to the next voxel
            }
