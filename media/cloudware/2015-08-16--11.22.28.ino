...
void updateAccelerometer() {
  ...
  for(int i=0;i<MAX_ACCEL;i++) {
    ...
    Serial_printf("accel[%d] = %d", i, accelerometer[i]);
  }
  
  if (accelerometer[2] < UPSIDE_DOWN) {
    Serial_printf("UPSIDE_DOWN");
  }
  
  if (accelerometer[0] > FACEPLANT) {
    Serial_printf("FACEPLANT");
  }
  
  if (accelerometer[1] < LEFT_SIDE) {
    Serial_printf("LEFT_SIDE");
  }
  
  if (accelerometer[1] > RIGHT_SIDE) {
    Serial_printf("RIGHT_SIDE");
  }

  if (accelerometer[2] > RIGHTSIDE_UP) {
    Serial_printf("RIGHTSIDE_UP");
  }
