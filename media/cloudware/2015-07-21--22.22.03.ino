    ...
    for(int i=0;i<pow(2,M)/2;i++)
    {
		...
		for(;y<cube.size;y++){
            ...
            if(smoothLEDs){
                ...
                pixelColor.red=pixelColor.red*(cube.size-y)/(cube.size);
                pixelColor.green=pixelColor.green*(cube.size-y)/(cube.size);
                pixelColor.blue=pixelColor.blue*(cube.size-y)/(cube.size);
            }
