...
void LED(int level, int row, int column, unsigned char red, unsigned char green, unsigned char blue){ //****LED Routine****LED Routine****LED Routine****LED Routine
	...
	//hopefully this all makes some sense?
  	// whoo-peeee...!
  	/*** yadda-yadda-yadda, let's do our thingy! */
...
void rainVersionTwo(){//****rainVersionTwo****rainVersionTwo****rainVersionTwo****rainVersionTwo****rainVersionTwo
	...
	for(addr=0; addr<64; addr++){
		x[addr]=rand()%8;	//random(8);
		y[addr]=rand()%8;	//random(8);
		z[addr]=rand()%8;	//random(8);
		xx[addr]=rand()%16;	//random(16);
		yy[addr]=rand()%16;	//random(16);
		zz[addr]=rand()%16;	//random(16);     
	}
	...
	while(millis()-start<20000){
		...
		for(addr=0; addr<leds; addr++){
			...
			// x[addr] = x[addr]+1;
			// y[addr] = y[addr]+1;
          	int rndUpperLimit=-100+(rand()%0);
			if(z[addr]<rndUpperLimit{	//random(-100,0)){
				x[addr]=rand()%8;	//random(8);
				y[addr]=rand()%8;	//random(8);
				
              	int select=rand()%3;	//random(3);
				if(select==0){
					xx[addr]=0;
					zz[addr]=rand()%16;	//random(16);
					yy[addr]=rand()%16;	//random(16);
