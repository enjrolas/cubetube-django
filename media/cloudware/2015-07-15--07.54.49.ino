...
for(int x=0;x<cube.size;x++)  //these three for statements will sweep through all 512 voxels of the cube and change the the voxel colors on each sweep
  for(int y=0;y<cube.size;y++)
    ...
    cube.setVoxel(x, pos, z, cube.colorMap((frame+50)%100,0,200));  //a core function from the L3D library for setting the color of a particular voxel in the Cube: http://goo.gl/u1iVEF         
    }
