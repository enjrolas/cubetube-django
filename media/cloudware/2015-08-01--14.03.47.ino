...
void LED(int level, int row, int column, unsigned char red, unsigned char green, unsigned char blue){ //****LED Routine****LED Routine****LED Routine****LED Routine
	...
	wholebyte=(row*64)+(column*8)+level;
  	voxel_color={adjustBrightness(red, dimFactor), adjustBrightness(green, dimFactor), adjustBrightness(blue, dimFactor)};
}//****LED routine end****
...
void ISR(){//***MultiPlex***
    ...
    for(int addr=0; addr<leds; addr++){
      ...
      strip.setPixelColor(wholebyte, strip.Color(adjustBrightness(base_color[addr].red, dimFactor), adjustBrightness(base_color[addr].green, dimFactor), adjustBrightness(base_color[addr].blue, dimFactor)));
    }
  	strip.show();
	//delay(1);
}
    ...
    for(int x=0;x<SIDE;x++)
        for(int y=0;y<SIDE;y++)
            for(int z=0;z<SIDE;z++){
                ...
                strip.setPixelColor(index, strip.Color(adjustBrightness(col.red, dimFactor), adjustBrightness(col.green, dimFactor), adjustBrightness(col.blue, dimFactor)));
            }
}
