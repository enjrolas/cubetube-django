...
void rainVersionTwo(){//****rainVersionTwo****rainVersionTwo****rainVersionTwo****rainVersionTwo****rainVersionTwo
    Color base_color1[64], base_color2[64];
	Point base_coord1[64], base_coord2[64];
    unsigned long start1, start2;	//for a millis timer to cycle through the animations
	...
	//while(millis()-start<20000){
		if(ledcolor<200){
			for(addr=0; addr<leds; addr++){
				...
				if(z[addr]<=0)
                  	//base_color1[addr]={10, 0, 1};
              		//base_coord1[addr]={x[addr], y[addr], z[addr]};
		...
		if(ledcolor>=200&&ledcolor<400){
			for(addr=0; addr<leds; addr++){
				...
				if(z[addr]<=0)
                  	//base_color2[addr]={10, 0, 0};
              		//base_coord2[addr]={x[addr], y[addr], z[addr]};
		...
		for(addr=0; addr<leds; addr++){
  		...
  		/*
        if(millis()-start1<100000)
         	for(addr=0; addr<leds; addr++)
          		LED(base_coord1[addr].z, base_coord1[addr].x, base_coord1[addr].y, base_color1[addr].red, base_color1[addr].green, base_color1[addr].blue);

  		if(millis()-start2<100000)
         	for(addr=0; addr<leds; addr++)
          		LED(base_coord2[addr].z, base_coord2[addr].x, base_coord2[addr].y, base_color2[addr].red, base_color2[addr].green, base_color2[addr].blue);
		*/
		for(addr=0; addr<leds; addr++){
