  ...
  for(int x = 0; x < maxSize; x++)
  {
   ...
   //     int zm = fmap(z,toFloat(-1),toFloat(1),toFloat(0),toFloat(maxSize-1))+0.5;
      //z = (int(phase)%maxSize + x) /2;
        ...
        {
          dim = .5;
          if (dist > floor(diameter + dilation)) 
          {
            dim = .5;
          }
          cube.setVoxel(x, y, z, Color(p * dim,fmap(dist,toFloat(0),toFloat(diameter + dilation),toFloat(0),toFloat(255)) * dim,100 * dim));
        }
      }
    }
  }
