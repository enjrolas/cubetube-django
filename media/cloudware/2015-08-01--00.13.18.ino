...
void setup(){
    ...
    randomSeed(seed); 
  	...
  	cube.begin();	//strip.begin();
	cube.background(voxel_color); // Initialize all pixels to 'off'
  	cube.show();	//strip.show();
...
void ISR(){//***MultiPlex***
	cube.background(black);
  	cube.setVoxel(voxel_coord, voxel_color);
    ...
    for(int addr=0; addr<leds; addr++){
      cube.setVoxel(base_coord[addr].z, base_coord[addr].y, 0, Color(base_color[addr].red, base_color[addr].green, base_color[addr].blue));
