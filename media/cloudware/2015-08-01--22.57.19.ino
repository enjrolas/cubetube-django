...
void ISR(){//***MultiPlex***
	// Blanking
  	background(black);	// Blanking
	//Color faded=lerpColor(voxel_color, black, random(0, wholebyte), random(0, wholebyte), wholebyte);
  	for(int addr=0; addr<leds; addr++){
      int blankingIndex=(xold[addr]*64)+(yold[addr]*8)+zold[addr];
      strip.setPixelColor(blankingIndex, strip.Color(constrain(--voxel_color.red, 0, maxBrightness), constrain(--voxel_color.green, 0, maxBrightness), constrain(--voxel_color.blue, 0, maxBrightness)));
    }
	// Drawing drop
