    ...
    for(int i=0;i<cube.size;i++)
        if(!salvos[i].dead)
            for(int j=0;j<MAX_POINTS;j++)
                cube.setVoxel(salvos[i].raindrops[j].raindrop.x, salvos[i].raindrops[j].raindrop.y, salvos[i].raindrops[j].raindrop.z, salvos[i].raindrops[j].color); //cube.lerpColor(purple, magenta, j, SPEED, MAX_POINTS)); //rainColor);
}
        ...
        for(int j=0;j<MAX_POINTS;j++)
        {
          	...
          	if(salvos[i].raindrops[j].speed>0)
            {
                ...
                else
                {
                    ...
                    {
                      	salvos[i].raindrops[j].color=cube.lerpColor(salvos[i].raindrops[j].color, black, j, SPEED, MAX_POINTS);
                        //salvos[i].raindrops[j].raindrop.x=-1;
                        //salvos[i].raindrops[j].raindrop.z=-1;
                      	//salvos[i].raindrops[j].flipped=false;
                    }
