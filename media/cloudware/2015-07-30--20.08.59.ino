...
void rainVersionTwo(){//****rainVersionTwo****rainVersionTwo****rainVersionTwo****rainVersionTwo****rainVersionTwo
	...
	while(millis()-start<20000){
		...
		if(ledcolor<200){
			for(addr=0; addr<leds; addr++){
				//LED(zold[addr], xold[addr], yold[addr], 0, 0, 0);
                wholebyte=(xold[addr]*64)+(yold[addr]*8)+zold[addr];
                voxel_color.red=0;
                voxel_color.green=5;
                voxel_color.blue=15;
                //Triggering the ISP() function to refresh the cube...
                ...
                digitalWrite(GAIN_CONTROL, HIGH);
              	...
              	z[addr]=constrain(z[addr], 0, 7);
              	switch(z[addr]){
					case 7:
						//LED(z[addr], x[addr], y[addr], 0, 5, 15);
                    	//     level,    row,    column
						wholebyte=(x[addr]*64)+(y[addr]*8)+z[addr];
                        voxel_color.red=0;
                        voxel_color.green=5;
                        voxel_color.blue=15;
                        voxel_coord.x=y[addr];
                        voxel_coord.y=z[addr];
                        voxel_coord.z=x[addr];
                    	break;
                  	case 6:
		...
		for(addr=0; addr<leds; addr++){
			xold[addr]=voxel_coord.z;	//x[addr];
			yold[addr]=voxel_coord.x;	//y[addr];
			zold[addr]=voxel_coord.y;	//z[addr];
		} 
