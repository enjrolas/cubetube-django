...
int CYCLE_INTERVAL = 10000; // milliseconds between restart
int lastSwap;
 ...
 drawColor = Color(0,0,100);
  ...
  for(int x = 0; x < maxSize; x++)
  {
    for(int y = 0; y < maxSize; y++)
    {
        ...
        z = round(map(toFloat(z),toFloat(-1),toFloat(1),toFloat(0),toFloat(maxSize-1)));
        if (x<=2 && y<=2) cube.setVoxel(x,y,z, drawColor);
    }
}
  phase += phaseInc; 
