...
//****These variables are for keeping the photon connected****
bool onlinePressed;
bool lastOnline;
UDP udp;
int lastUpdated;
...
void setup(){
    ...
    digitalWrite(GAIN_CONTROL, HIGH);
	attachInterrupt(GAIN_CONTROL, ISR, CHANGE);
...
void LED(int level, int row, int column, unsigned char red, unsigned char green, unsigned char blue){ //****LED Routine****LED Routine****LED Routine****LED Routine
	...
	wholebyte=(row*64)+(column*8)+level;
  	...
  	//Triggering the ISP() function to refresh the cube...
    digitalWrite(GAIN_CONTROL, LOW);
    digitalWrite(GAIN_CONTROL, HIGH);
  	//cube.setVoxel(column, level, row, Color(red, green, blue));
    //cube.show();
...
void initButtons() {
  ...
  void (*check)(void) = &onlineOfflineSwitch;
  attachInterrupt(INTERNET_BUTTON, (void (*)())check, CHANGE);

  void (*wifi)(void) = &joinWifi;
...
void onlineOfflineSwitch() {
  ...
  if((onlinePressed) && (lastOnline)) {
    //marked as 'online'
    lastOnline = onlinePressed;
    Spark.connect();
  } else if((onlinePressed) && (lastOnline)) {
    // marked as 'offline'
    lastOnline = onlinePressed;
    Spark.disconnect();
  }
    ...
    for(int x = 0; x < SIDE; x++) {
      for(int y = 0; y < SIDE; y++) {
        for(int z = 0; z < SIDE; z++) {
          ...
          strip.setPixelColor(index, strip.Color((data[index]&0xE0)>>2, (data[index]&0x1C)<<1, (data[index]&0x03)<<4));	//setVoxel(x, y, z, pixelColor);
        }
      }
    }
  }
