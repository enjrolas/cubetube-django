 ...
 * *****************************/
float phase = 0.35;
float phaseIncrement = 0.055; // Controls the speed of the moving points. Higher == faster
float colorStretch = 0.025; // Higher numbers will produce tighter color bands 
...
void zPlasma() {
    ...
    Point p1 = {(sin(phase * 1.505) + 1.0) * 4, (sin(phase * 1.310) + 1.0) * 4.0, (sin(phase * 1.380) + 1.0) * 4.0};
	...
	for (col = 0; col < cube.size; col++) {
		...
		float color_4 = 1/(tan(distance1 * distance2 * colorStretch))*(sin(distance1 * distance2 * colorStretch)*0.25+1) + 2.0 * 0.25;
