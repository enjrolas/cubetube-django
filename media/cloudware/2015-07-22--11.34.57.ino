...
Color _colorMap(float _val, float _min, float _max)
{
  ...
  if (_val<=_range/6)
    return(lerpColor(colors[0], colors[1], _val, 0, _range/6));
  else if (_val<=2*_range/6)
    return(lerpColor(colors[1], colors[2], _val, _range/6, 2*_range/6));
  else if (_val<=3*_range/6)
    return(lerpColor(colors[2], colors[3], _val, 2*_range/6, 3*_range/6));
  else if (_val<=4*_range/6)
    return(lerpColor(colors[3], colors[4], _val, 3*_range/6, 4*_range/6));
  else if (_val<=5*_range/6)
    return(lerpColor(colors[4], colors[5], _val, 4*_range/6, 5*_range/6));
  else
    return(lerpColor(colors[5], colors[0], _val, 5*_range/6, _range));
}
