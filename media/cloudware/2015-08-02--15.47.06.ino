...
void setup(){
	...
	attachInterrupt(REFRESH_CONTROL, ISR, CHANGE);
  	tone(REFRESH_CONTROL, 3600, 0);	// Sets our refresh rate to 1.8KHz
  	initCloudButton();
	...
	//while(millis()-start<20000){
		if(ledcolor<200){
			for(addr=0; addr<leds; addr++){
				...
				if(z[addr]<=0){
                  	base_color[addr]={random(10, 16), 0, 1};	//base_color[addr]={10, 0, 1};
		...
		if(ledcolor>=200&&ledcolor<400){
			for(addr=0; addr<leds; addr++){
				...
				if(z[addr]<=0){
                  	base_color[addr]={random(10, 16), random(0, 2), 0};	//base_color[addr]={10, 0, 0};
              		base_coord[addr]={z[addr], x[addr], y[addr]};
