...
int fadingMax=25;
int startAt;	//[0=base|1=center|2=top|3=random]
...
void setup() {
  ...
  // Where do we want our raindrops to start from?
  startAt=1;	//[0=base|1=center|2=top|3=random]
            ...
            for(int j=0;j<MAX_POINTS;j++)
              	if(!salvos[i].raindrops[j].dead){
                	cube.setVoxel(salvos[i].raindrops[j].raindrop.x, salvos[i].raindrops[j].raindrop.y, salvos[i].raindrops[j].raindrop.z, salvos[i].raindrops[j].color);
                  	...
                  	Color fade;
                  	fade.red=salvos[i].raindrops[j].color.red>0 ? salvos[i].raindrops[j].color.red-- : 0;
                  	fade.green=salvos[i].raindrops[j].color.green>0 ? salvos[i].raindrops[j].color.green-- : 0;
                  	fade.blue=salvos[i].raindrops[j].color.blue>0 ? salvos[i].raindrops[j].color.blue-- : 0;
                  	if(salvos[i].raindrops[j].speed>0)
                      	cube.setVoxel(salvos[i].raindrops[j].raindrop.x, salvos[i].raindrops[j].raindrop.y-1, salvos[i].raindrops[j].raindrop.z, fade);
                	else
                      	cube.setVoxel(salvos[i].raindrops[j].raindrop.x, salvos[i].raindrops[j].raindrop.y+1, salvos[i].raindrops[j].raindrop.z, fade);
                }
}
