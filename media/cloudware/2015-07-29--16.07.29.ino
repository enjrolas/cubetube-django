...
void LED(int level, int row, int column, unsigned char red, unsigned char green, unsigned char blue){ //****LED Routine****LED Routine****LED Routine****LED Routine
	...
	//int whichbyte = int(((level*64)+(row*8)+column)/8);
  	...
  	//strip.setPixelColor(wholebyte, strip.Color(red, green, blue));
  	cube.setVoxel(row*row, column*column, level*level, Color(red, green, blue));
  	//cube.setVoxel(level, row, column, Color(red, green, blue));
  	//Z*cube.setVoxel(column, level, row, Color(red, green, blue));
