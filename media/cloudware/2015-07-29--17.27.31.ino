...
void rainVersionTwo(){//****rainVersionTwo****rainVersionTwo****rainVersionTwo****rainVersionTwo****rainVersionTwo
	...
	while(millis()-start<100){//20000){
		...
		if(ledcolor<200){
			for(addr=0; addr<leds; addr++){
				//LED(zold[addr], xold[addr], yold[addr], 0, 0, 0);
              	cube.setVoxel(xold[addr], yold[addr], zold[addr], black);
				if(z[addr]>=7) cube.setVoxel(x[addr], y[addr], z[addr], Color(0, 5, 15));
					//LED(z[addr], x[addr], y[addr], 0, 5, 15);
				if(z[addr]==6) cube.setVoxel(x[addr], y[addr], z[addr], Color(0, 1, 9));
					//LED(z[addr], x[addr], y[addr], 0, 1, 9);
				if(z[addr]==5) cube.setVoxel(x[addr], y[addr], z[addr], Color(0, 0, 10));
					//LED(z[addr], x[addr], y[addr], 0, 0, 10);
				if(z[addr]==4) cube.setVoxel(x[addr], y[addr], z[addr], Color(1, 0, 11));
					//LED(z[addr], x[addr], y[addr], 1, 0, 11); 
				if(z[addr]==3) cube.setVoxel(x[addr], y[addr], z[addr], Color(3, 0, 12));
					//LED(z[addr], x[addr], y[addr], 3, 0, 12);
				if(z[addr]==2) cube.setVoxel(x[addr], y[addr], z[addr], Color(10, 0, 15));
					//LED(z[addr], x[addr], y[addr], 10, 0, 15);
				if(z[addr]==1) cube.setVoxel(x[addr], y[addr], z[addr], Color(10, 0, 10));
					//LED(z[addr], x[addr], y[addr], 10, 0, 10);
				if(z[addr]<=0) cube.setVoxel(x[addr], y[addr], z[addr], Color(10, 0, 1));
		...
		if(ledcolor>=200&&ledcolor<300){
			for(addr=0; addr<leds; addr++){
				//LED(zold[addr], xold[addr], yold[addr], 0, 0, 0);
              	cube.setVoxel(xold[addr], yold[addr], zold[addr], black);
				if(z[addr]>=7) cube.setVoxel(x[addr], y[addr], z[addr], Color(15, 15, 0));
					//LED(z[addr], x[addr], y[addr], 15, 15, 0);
				if(z[addr]==6) cube.setVoxel(x[addr], y[addr], z[addr], Color(10, 10, 0));
					//LED(z[addr], x[addr], y[addr], 10, 10, 0);
				if(z[addr]==5) cube.setVoxel(x[addr], y[addr], z[addr], Color(15, 5, 0));
					//LED(z[addr], x[addr], y[addr], 15, 5, 0);
				if(z[addr]==4) cube.setVoxel(x[addr], y[addr], z[addr], Color(15, 2, 0));
					//LED(z[addr], x[addr], y[addr], 15, 2, 0); 
				if(z[addr]==3) cube.setVoxel(x[addr], y[addr], z[addr], Color(15, 1, 0));
					//LED(z[addr], x[addr], y[addr], 15, 1, 0);
				if(z[addr]==2) cube.setVoxel(x[addr], y[addr], z[addr], Color(15, 0, 0));
					//LED(z[addr], x[addr], y[addr], 15, 0, 0);
				if(z[addr]==1) cube.setVoxel(x[addr], y[addr], z[addr], Color(12, 0, 0));
					//LED(z[addr], x[addr], y[addr], 12, 0, 0);
				if(z[addr]<=0) cube.setVoxel(x[addr], y[addr], z[addr], Color(10, 0, 0));
...
void LED(int level, int row, int column, unsigned char red, unsigned char green, unsigned char blue){ //****LED Routine****LED Routine****LED Routine****LED Routine
	...
	if(blue>15)  blue=15;  
  	...
  	cube.setVoxel(row, column, level, Color(red, green, blue));
