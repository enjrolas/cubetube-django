...
// to load this viz in your cube. Doing so means you have
// READ & UNDERSTOOD the side effects it may cause, and you
// are comfortable with dealing with issues to your photon/core.
// ** This was tested in a PHOTON - I have no idea if this will
// work the same in different (older-generation) processors.
// If this is your case, USE IT AT YOUR OWN RISK.
// ** END DISCLAIMER **
// ** END DISCLAIMER **
// ** END DISCLAIMER **
//
...
// ** #2:
//   Because of the way the cube is updated, I have had
//   various problems running the viz while keeping the cube
//   connected. So I decided to leave the "connect-to-cloud"
//   part disabled for the time being. You will notice the
//   code for that is still there, but it's not being called.
// ** #3:
//   Be advised! Once you load this viz into the cube, you'll
//   immediately notice 2 things: your photon will start to 
//   "breathe white" and the message "No cores online :(" will
//   appear on your device list dropdown in cubetube.
// ** #4:
//	 If that's something you cannot live with, you've got two
//	 choices: either DO NOT load this viz, or after you've had
//	 enough of this viz, bring your photon to "firmware reset"
//	 mode (this is NOT the same as a factory reset - it'll only
//	 erase the code from your photon, that is, this viz will
//	 be erased from its flash memory).
//	 Look in here for details on how to do it: 
//	    http://docs.particle.io/photon/modes/
//
// ** I'd appreciate help from anyone who could shed some light
//	  into this connection issue. Feel free to comment here or
//    on the forums.
// ** HAVE PHUN **
...
void setup(){
	...
	attachInterrupt(REFRESH_CONTROL, ISR, CHANGE);
  	tone(REFRESH_CONTROL, 1000, 0);	// Sets our refresh rate to 1KHz
