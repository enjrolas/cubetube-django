...
public:
  ...
  void update(float deltaSecs)
  {
    ...
    if (position[0] <= 0 && velocity[0] < 0)
    {
      ...
      float noiseY = (randFloat() * 2 - 1.0) * motionNoise;
      float noiseZ = (randFloat() * 2 - 1.0) * motionNoise;
      velocity[1] += noiseY * velocity[1];
      velocity[2] += noiseZ * velocity[2];
      normaliseSpeed();
    }
    ...
    if (position[1] <= 0 && velocity[1] < 0)
    {
      ...
      float noiseX = (randFloat() * 2 - 1.0) * 1.0;
      float noiseZ = (randFloat() * 2 - 1.0) * 1.0;
      velocity[0] += noiseX * velocity[0];
      velocity[2] += noiseZ * velocity[2];
      normaliseSpeed();
    }
    ...
    if (position[2] <= 0 && velocity[2] < 0)
    {
      ...
      float noiseX = (randFloat() * 2 - 1.0) * motionNoise;
      float noiseY = (randFloat() * 2 - 1.0) * motionNoise;
      velocity[0] += noiseX * velocity[0];
      velocity[1] += noiseY * velocity[1];
      normaliseSpeed();
