...
int fadingMax=25;
int startAt=1;	//[0=base|1=center|2=random]
        ...
        for(int j=0;j<numDrops;j++) {
            ...
            salvos[i].raindrops[j].raindrop.z=rand()%8;
              ...
              case 0:
                ...
                break;
              case 1:
          		salvos[i].raindrops[j].raindrop.y=salvos[i].raindrops[j].speed>0 ? cube.size/2 : (cube.size/2)+1;
                break;
              case 2:
          		salvos[i].raindrops[j].raindrop.y=salvos[i].raindrops[j].speed>0 ? 1 : 8;
                break;
            }
        ...
        for(int j=0;j<MAX_POINTS;j++) {
            salvos[i].raindrops[j].raindrop.y+=salvos[i].raindrops[j].speed;
          	if(salvos[i].raindrops[j].speed>0) {
                ...
                if(salvos[i].raindrops[j].raindrop.y>1) {
                  	if(salvos[i].raindrops[j].flipped)
                  		salvos[i].raindrops[j].color=cube.lerpColor(salvos[i].raindrops[j].color, black, i, abs(salvos[i].raindrops[j].speed), fadingMax);
                }
                else {
