...
void ISR(){//***MultiPlex BAM***MultiPlex BAM***MultiPlex BAM***MultiPlex BAM***MultiPlex BAM***MultiPlex BAM***MultiPlex BAM
	...
	if(redrawBase)
      for(addr=0; addr<leds; addr++){
        int index=(base_coord[addr].x*64)+(base_coord[addr].y*8)+base_coord[addr].z;
    	strip.setPixelColor(index, strip.Color(base_color[addr].red, base_color[addr].green, base_color[addr].blue));
      }
  	strip.show();
	delay(2);
}
		...
		if(ledcolor<200){
			for(addr=0; addr<leds; addr++){
				...
				if(z[addr]<=0){
              		...
              		base_coord[addr]={x[addr], y[addr], z[addr]};
