...
def viz_most_flashed(request):
    ...
    try:
        if "sqlite" in db_engine:
            grouped_query=Viz.objects.raw("SELECT DISTINCT id, STRFTIME(\'%%m/%%d/%%Y\',lastFlashed) AS \'fmtLastFlashed\', LTRIM(RTRIM(name)) AS \'name\', MAX(views) AS \'count\' FROM \'viz_viz\' WHERE (vizType = \'L3D\' AND published = \'true\' AND (lastFlashed >= \'{0}\' AND lastFlashed < \'{1}\')) GROUP BY STRFTIME(\'%%m/%%d/%%Y\',lastFlashed) ORDER BY STRFTIME(\'%%m/%%d/%%Y\',lastFlashed) ASC".format(startDate.strftime('%Y-%m-%d'), endDate.strftime('%Y-%m-%d')))
        else:
