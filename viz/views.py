...
def unique_daily_users(request):
    ...
    log.debug("startDate: %s" % startDate.strftime('%Y-%m-%d'))
    log.debug("endDate: %s" % endDate.strftime('%Y-%m-%d'))
    log.debug("SQL QUERY: %s" % CubeUser.objects.get_queryset().extra(select={'day':'DATE_FORMAT(lastActivity,\'%%d\')','fmtLastActivity':'DATE_FORMAT(lastActivity,\'%%m/%%d/%%Y\')'}, where=['vizType = \'%s\' AND (lastActivity >= \'%s\' AND lastActivity < \'%s\')' % ("L3D", startDate.strftime('%Y-%m-%d'), endDate.strftime('%Y-%m-%d'))]).values('day').annotate(count=Count('pk')).values('fmtLastActivity','count').order_by("fmtLastActivity").query.__str__())
    
    grouped_query=CubeUser.objects.get_queryset().extra(select={'day':'DATE_FORMAT(lastActivity,\'%%d\')','fmtLastActivity':'DATE_FORMAT(lastActivity,\'%%m/%%d/%%Y\')'}, where=['vizType = \'%s\' AND (lastActivity >= \'%s\' AND lastActivity < \'%s\')' % ("L3D", startDate.strftime('%Y-%m-%d'), endDate.strftime('%Y-%m-%d'))]).values('day').annotate(count=Count('pk')).values('fmtLastActivity','count').order_by("fmtLastActivity")
