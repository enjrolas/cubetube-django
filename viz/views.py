...
def viz_flashed(request):
    ...
    startDate = datetime.datetime.strptime("%s %s %s" % (int(month), 1, today.year),"%Y-%m-%d").date()
    endDate = datetime.datetime.strptime("%s %s %s" % (next_month, 1, next_year),"%Y-%m-%d").date()
